...
NGRAM_MIN = 1
NGRAM_MAX = 3  # Turned into constants after checking more than one parameters, explained in folhinhas
MIN_FREQ_TERMS = 1  # Turned into constants after checking more than one parameters, explained in folhinhas
...
all_names = vec_prev.get_feature_names()
vocabulary = []
for i in range(0, len(all_names)):
    for j in range(0, len(keywords)):
        if all_names[i] == keywords[j]:
            vocabulary.append(keywords[j])
