...
class LSA:
    ...
    def process_phrase(self, index, keywords, set_):
        ...
        examples.append(set_.phrases[index])
        tfidf_utterance = numpy.array(self.tf_idf(examples, self.features_utterance))
        tfidf_keywords = numpy.array(self.tf_idf(examples, keywords))
        x = numpy.round(self.eliminate_dimensions(numpy.concatenate([tfidf_utterance, tfidf_keywords], axis=1)), 10)
        set_.set_lsa_result(index, x[-1])
    ...
    def train_lsa(self, keywords, set_):
        tfidf_utterance = numpy.array(self.tf_idf(set_.phrases, self.features_utterance))
        tfidf_keywords = numpy.array(self.tf_idf(set_.phrases, keywords))
        x = numpy.round(self.eliminate_dimensions(numpy.concatenate([tfidf_utterance, tfidf_keywords], axis=1)), 10)
        for i in range(len(set_.phrases)):
            set_.set_lsa_result(i, x[i])
